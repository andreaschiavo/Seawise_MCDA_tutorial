...
The **mcda.R** script needs a specific input data format and must have the following structure:

column1: *"time_period"*, column2: *"scenario"*, (optional) column3: *"other colums(e.g. quantile level)"*, column4-columnN: <name_of_the_criteria>

If one or more criteria have sub_criteria:

column1: *"time_period"*, column2: *"scenario"*, (optional) column3: *"quantile level"*, column4 *"sub_criteria"*, column5-n: <name_of_the_criteria>
...
Load the input data and calculate the utitlities of the critaria based on the MCDA option file (see [last section](05_genOptionsFile.html))
Utilities are calculated based on utility functions and reference points defined for each criteria into the options file.
...
It is needed to add a filter to the input dataset and extract one single value of the criteria for each combination of time-period and management scenario (alternatives). Without this additional argument the function would filter only by time-period and scenario (default filter), returning a dataset with quantile levels (if present in the dataset) and failing the calculation of the utilities.
...
Aggregate the criteria into the macro-criteria. Macro-criteria are then passed to the *run_maut* function.
